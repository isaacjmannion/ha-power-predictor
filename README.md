...
| `Peak Start Hour` | **Local** clock hour the peak period begins (0–23, inclusive) | 9 |
| `Peak End Hour` | **Local** clock hour the peak period ends (0–23, inclusive) | 22 |
| `Peak Quantile` | Quantile applied during peak hours (higher = more conservative) | 0.75 |
| `Off-Peak Quantile` | Quantile applied during off-peak hours (0.50 = median) | 0.50 |
...
| `Temperature Influence Weight` | Relative influence of temperature and its lag features (`0` = off, `1` = neutral; raise if temperature drives your load, e.g. HVAC) | 0.5 |
...
- **Change — Peak/Off-Peak window is now local time:** the `Peak Start/End Hour` are interpreted on your **local** clock (previously UTC internally), so the conservative Peak Quantile lands on your real high-usage window regardless of timezone, and it stays aligned across daylight-saving changes. The rest of the pipeline is unchanged (still UTC under the hood).
...
- **Retuned defaults:** the default model parameters were chosen from walk-forward backtesting — `hour harmonics 3`, `alpha 0.1`, time weight `2.0`, lag weight `0.5`, and a light temperature weight (`Temperature Influence Weight 0.5`, `Temperature Lag Features 0`). Temperature was only marginally useful on the load these were tuned against; raise it toward `1.0` (with 1–2 temp lags) if your load is weather-driven. Defaults are a sensible starting point, not a universal optimum — tune for your home with the export + backtest workflow.
